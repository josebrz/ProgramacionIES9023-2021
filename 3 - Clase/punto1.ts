...
class Auto{
    ...
    constructor(marca:string, modelo:string, potencia:string, velocidad_max:number){
        this._marca = marca;
        this._modelo = modelo;
    ...
    tiempoLlegada(km:number){
        function numToHora (num:number): string{
...
const mostrarAutos = (arrAutos: Auto[]) => {
    let aux = 0
...
const calcularTiempo = (arrAutos: Auto[], km:number) => {
...
class CuentaBancaria{
    ...
    consultarFondo():string{
        return `Su cuenta tiene fondos: $${this._fondo}`
    }

    asignar(monto:number):string{
        if(monto > 500000){
